pub mod bridge_v2 {
    ...
    const PROXY_CONTRACT: &str = "zera_bridge_proxy_1";
    ...
    fn get_usd_value(amount_str: String, contract_id: String, token_price: String) -> u64 {
        unsafe {
            ...
            if !authorized || rate == U256::zero() {
                ...
                let divisor = types::string_to_u256(ONE_USD.to_string());
                let usd_value: U256 = (((amount * token_price_u256) / denomination) / divisor) * 100;
...
fn store_tx_signature(tx_signature: String) {
    unsafe {
        smart_contracts::delegate_store_state(
...
fn check_tx_signature(tx_signature: String) -> Result<(), SimpleErr> {
    unsafe {
        let tx_signature_state = smart_contracts::delegate_retrieve_state(format!("{}{}", TX_SIGNATURE_KEY.to_string(), tx_signature.clone()), PROXY_CONTRACT.to_string());
...
fn check_and_update_rate_limit(
    ...
    unsafe {
        ...
        let amount_usd = get_usd_value(amount_str.clone(), contract_id.clone(), token_price.clone());
...
fn verify_guardian_signatures(
    ...
    unsafe {
        ...
        let used_sig = smart_contracts::delegate_retrieve_state(used_sig_key.clone(), PROXY_CONTRACT.to_string());
        ...
        if valid_signatures >= required_threshold {
            ...
            smart_contracts::delegate_store_state(used_sig_key.clone(), "used".to_string(), PROXY_CONTRACT.to_string());
            return Ok(());
        }
...
pub fn lock_zera(contract_id: String, amount: String, solana_address: String) {
    unsafe {
        ...
        if !is_valid_solana_address(&solana_address) {
...
pub fn burn_sol(contract_id: String, amount: String, solana_address: String) {
    unsafe {
        ...
        let mint_id = smart_contracts::delegate_retrieve_state(mint_id_key.clone(), PROXY_CONTRACT.to_string());
...
pub fn mint_sol(
    ...
    unsafe {
        ...
        let contract_id = smart_contracts::delegate_retrieve_state(
            format!(
                "{}{}",
...
pub fn create_sol(
    ...
    unsafe {
        ...
        let contract_id_state = smart_contracts::delegate_retrieve_state(exist_key.clone(), PROXY_CONTRACT.to_string());
        ...
        if contract_id_state.is_empty() {
            ...
            let mint_id_key = format!("{}{}", MINT_ID_KEY.to_string(), contract_id.clone());
            smart_contracts::delegate_store_state(exist_key.clone(), contract_id.clone(), PROXY_CONTRACT.to_string());
            save_state(SYMBOL_CONFIG_KEY, &symbol_config);
            smart_contracts::delegate_store_state(mint_id_key.clone(), mint_id.clone(), PROXY_CONTRACT.to_string());
...
pub fn update_guardian_state(guardians: String, guardian_threshold: String) {
    unsafe {
        ...
        if guardian_threshold_value > u8::MAX as u64 {
            smart_contracts::emit(format!(
                "Failed: Guardian threshold cannot exceed {}",
                u8::MAX
            ));
            return;
        }
        ...
        if guardian_threshold_value > guardians_vec.len() as u64 {
            smart_contracts::emit(
                format!("Failed: Threshold ({}) cannot be greater than number of guardian keys ({})", guardian_threshold_value, guardians_vec.len() as u64)
            );
            return;
        }
        ...
        guardian_state.guardian_threshold = guardian_threshold_value as u8;
